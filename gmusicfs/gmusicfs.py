    ...
    def get_tracks(self, get_size=False):
        ...
        if get_size and self.library.true_file_size:
            for t in self.__tracks:
                if 'bytes' not in t:
                    r = urllib.request.Request(self.get_track_stream(t)[0])
...
class GMusicFS(LoggingMixIn, Operations):
    ...
    def read(self, path, size, offset, fh):
        ...
        if artist_album_track_m and (int(u.headers['Content-Length']) < (offset + size)):
            ...
            id3v1 = struct.pack("!3s30s30s30s4s30sb".encode("utf-8"), b'TAG', str(track['title']).encode("utf-8"), str(track['artist']).encode("utf-8"),
                                str(track.get('album','')).encode("utf-8"), str(0).encode("utf-8"), str(track.get('comment','')).encode("utf-8"), 12)
